...
test('no tax on zero-profit bet', () => {
  ...
  // Profit = 100 * 1.0 - 100 = 0 → early return, no grossWinnings field
  assert.equal(result.taxableAmount, 0);
  assert.equal(result.tax, 0);
...
test('cumulative winnings accumulate correctly across multiple bets', () => {
  // Bet 1: profit 6000, cumulative before = 0
  const r1 = calculateBetTax(2000, 4.0, 0);
  assert.equal(r1.grossWinnings, 6000);
  assert.equal(r1.tax, 0); // threshold not reached

  // Bet 2: profit 6000, cumulative before = 6000
  // remaining threshold = 10000 - 6000 = 4000
  // taxable = 6000 - 4000 = 2000, tax = 60
  const r2 = calculateBetTax(2000, 4.0, 6000);
  assert.equal(r2.grossWinnings, 6000);
  assert.equal(r2.tax, 60);

  // Bet 3: profit 6000, cumulative before = 12000 (already over threshold)
  // remaining = 0, taxable = 6000, tax = 180
  const r3 = calculateBetTax(2000, 4.0, 12000);
  assert.equal(r3.grossWinnings, 6000);
  assert.equal(r3.tax, 180);

  // Bet 4: profit 500, cumulative before = 18000
