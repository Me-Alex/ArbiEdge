/**
 * Server-side bet tracker with JSONL persistence and analytics.
 *
 * Bets are stored as newline-delimited JSON in data/bets.jsonl.
 * Each record is a full bet object (see createBet).
 *
 * Analytics are computed on-demand from the settled bet set.
 */
...
class BetTracker {
  ...
  /**
   * Read all bets from the JSONL log.
   */
  readAll() {
  ...
  /**
   * Rewrite the entire JSONL log (used for updates/deletes).
  ...
  /**
   * Update a bet (e.g. add closing odds, change stake).
   */
  update(id, updates) {
    const bets = this.readAll();
    const bet = bets.find((b) => b.id === id);
  ...
  /**
   * Compute analytics from settled bets.
   *
   * Returns:
   * - totalBets, settledBets, pendingBets
   * - totalStake, totalReturn, netProfit
   * - roi (netProfit / totalStake)
   * - hitRate (won / settled)
   * - avgOdds
   * - clv (average closing line value: closingOdds vs odds)
   * - byBookmaker: { name: { bets, stake, profit, roi, hitRate } }
   * - byMarket: { market: { bets, stake, profit, roi, hitRate } }
   * - byCompetition: { comp: { bets, stake, profit, roi, hitRate } }
   */
  analytics({ sport } = {}) {
    let bets = annotateTaxFields(this.readAll());
    if (sport) bets = bets.filter((b) => b.sport === sport);
    ...
    for (const bet of settled) {
      ...
      // CLV: compare placed odds to closing odds
