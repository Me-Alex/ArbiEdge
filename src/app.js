...
const {
  ...
  coverageSummary,
} = require('./bookmaker-coverage');
const { getAllOpportunities, getValueBets, describeMarket } = require('./formula-engine');
const { createLogger } = require('./logger');
const { BetTracker, VALID_STATUSES } = require('./bet-tracker');
const { RateLimiter } = require('./rate-limiter');
const { runAutoSettle, getPendingPicks, readAiPicks } = require('./ai-pick-settler');
const { allSportOptions, sportCanonical, parseSportList } = require('./sport-config');
...
function createApp({
  ...
  const betTracker = new BetTracker({ logPath: betLogPath });
  ...
  app.get('/api/metrics', (request, response) => {
    response
      ...
      .send(metricsText(serviceDiagnostics(oddsService)));
  });
